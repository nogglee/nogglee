(function () {
	...
	// 4. 기타 인앱 처리 (iOS vs Android 분기)
	if (useragt.match(/inapp|naver|snapchat|wirtschaftswoche|thunderbird|instagram|everytimeapp|whatsApp|electron|wadiz|aliapp|zumapp|iphone(.*)whale|android(.*)whale|kakaostory|band|twitter|DaumApps|DaumDevice\/mobile|FB_IAB|FB4A|FBAN|FBIOS|FBSS|trill|SamsungBrowser\/[^1]/i)) {
		if (/iphone|ipad|ipod/i.test(useragt)) {
			// iOS Safari 유도 안내 화면 렌더링
			document.body.innerHTML = '<h2>Safari로 접속해 주세요</h2> ...';
		} else {
			// Android Chrome으로 강제 intent 리디렉션
